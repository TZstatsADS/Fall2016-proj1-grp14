...
quantile(hdata$income, probs = seq(0, 1, 0.1))
quantile(hdata$child, probs = seq(0, 1, 0.1))
ggplot(hdata ,aes(factor(child),income))+geom_violin(aes(fill=child),scale = "count",trim = FALSE)

# Cut off the dataset and plot the data
a <- filter(hdata, income <= 200000 & child <= 5)
b <- filter(hdata,income <= 200000)

# Violin plot and dotplot
...
# Five states with high children number
