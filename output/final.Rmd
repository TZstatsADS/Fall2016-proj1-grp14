...
##Map of number of children in household (here dark red means high birth rate and white means low)
...
result$FES <- as.factor(result$FES)
levels(result$FES) <- c('Both LF', 'HLF', 'WLF', 'N', 'MaleLF', 'MaleNT', 'FemaleLF', 'FemaleNT')
...
```
In the following part, we use ggplot to show the percentage and total NOC under different family type and employment status. From these plots, we can find that the family type that "Only Husband in Labor Force" tends to have more number of children. The family type, "Female in Labor Force", tends to have less number of children.
...
plot(Sankey1)
```
Referring to the inital state analysis in the first part, we compared the Family Type and Employment Status in states which have more children and states which have less children. 
...
```
Comparing these two graphs, we can find that states, which have higher number of children, "Both in Labor Force" and "Only husband in Labor force" have larger percentage. However, states, which have lower number of children, "Both in Labor Force" and "Female in Labor Force" have larger percentage.
...
plot_ly(c,x=income,y=fitted(loess(avg_child ~ income))) %>%
  layout (title = "Average Number of Children for Different Household Income")
```
...
```
After we got the HCR, plotted the histogram to check the overall distribution: we can conclude from the graph that 70% of the house-related cost stays in the range between 12.5% and 50%.
...
```
Then we plotted the relationship between NOC and cost ratio in 5 levels.
...
After we got the average number of children in different states,
